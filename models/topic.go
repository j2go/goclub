...
type Topic struct {
	...
	LastReplyTime    time.Time `orm:"column(last_reply_time);type(datetime);null"`
	...
	TopStatus        int       `orm:"column(top_status)"`
	VisitTimes       int       `orm:"column(visit_times)"`
	TabId            int       `orm:"column(tab_id);null"`
}
